...
with open("final/preprocess.pickle", "rb") as q:
    [jpg_word_dict, lib_ids, lib, triplet] = pickle.load(q)

netvladids = pd.read_csv("final/data_68_.csv")["imagesouce"].to_numpy()
path = args.path
Writer = SummaryWriter("final/tbx/" + args.logid)


def filter(triplets, ids):
    y = []
    for i in triplets:
        # print(i)
        if i[1] in ids:
...
triplet = filter(triplet, netvladids)
